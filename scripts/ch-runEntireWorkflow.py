...
training_files="../simple"
test_files='../files/ch-test/'
temp_dir='./temp/'
output_dir='../mlModel'
...
#Inference
subprocess.run(['python3', 'ch-collectFeatures.py', 'test', test_files])
subprocess.run(['python3', 'combineFeatures.py','test',output_dir+'/test.csv'])

#Model
subprocess.run(['python3', 'ch-train.py'], cwd=ml_model)
subprocess.run(['python3', 'ch-infer.py'], cwd=ml_model)
