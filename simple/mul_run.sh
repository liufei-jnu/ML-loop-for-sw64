...
for file in *.c *.cpp; do
    if [ -f "$file" ]; then
        ...
        # Record features
        echo "Output for $file:" >> features.csv
        opt --disable-output -load-pass-plugin="${PATH2LIB}" -passes="${PASS}" "${file%.c}.profdata.bc" >> features.csv 2>&1
        echo "" >> features.csv
    fi
done
    ...
    if [ -d "$dir" ]; then
        ...
        if [ ${#bc_files[@]} -gt 0 ]; then
            ...
            if [ -f "default.profraw" ]; then
                ...
                # Record features
                project_name="${dir%/}"
                echo "Output for $project_name:" >> ../features.csv
                opt --disable-output -load-pass-plugin="${PATH3LIB}" -passes="${PASS}" combined.profdata.bc >> ../features.csv 2>&1
                echo "" >> ../features.csv
            else
