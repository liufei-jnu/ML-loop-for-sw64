            ...
            for (Loop *loop : li)
            {
                ...
                for (auto bb : loop->getBlocks())
                {
                    for (auto &instr : *bb)
                    {
                        ...
                        uint64_t blockCount = bfi.getBlockProfileCount(instr.getParent()).getValue();
...
while (llvm::Loop *parentLoop = currentLoop->getParentLoop()) {
    currentLoop = parentLoop;
}
visitedOuterLoops.push_back(currentLoop);
